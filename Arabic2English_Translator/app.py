...
service = BaseService("a2e_translator")  # Changed service name
app = service.app
...
# Add TranslationStatus model and status storage
...
FORMALITY_PROMPTS = {
    "formal": "Translate to formal English (provide ONLY the English translation):",
    "neutral": "Translate to English (provide ONLY the English translation):",
    "informal": "Translate to informal English (provide ONLY the English translation):"
}
...
Reply with ONLY the English translation, no explanations or Arabic text."""
    
    translation = generate_translation(prompt)
    
    # Clean up the response to get only English text
    # Remove any Arabic text and extra whitespace
    english_only = ''.join(char for char in translation if ord(char) < 1536 or char.isspace())
    english_only = ' '.join(english_only.split())
    
    result = {"translation": english_only, "formality": request.formality}
...
@api_router.get("/translate/ar2en/status/{request_id}")
